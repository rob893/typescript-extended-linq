...
describe.each([...getEnumerables()])('rightJoinHeterogeneous', (src, enumerable) => {
  it.each([[1, 2, 3], new Set([1, 2, 3]), '123', new Map()])('should return an Enumerable', collection => {
    const result = enumerable(src<unknown>(collection)).rightJoinHeterogeneous<unknown, unknown, unknown>(
      [],
      x => x,
      x => x,
      x => x,
...
describe.each([...getEnumerables()])('rightJoinHomogeneous', (src, enumerable) => {
  it.each([[1, 2, 3], new Set([1, 2, 3]), '123', new Map()])('should return an Enumerable', collection => {
    const result = enumerable(src<unknown>(collection)).rightJoinHomogeneous<unknown, unknown>(
      [],
      x => x,
      x => x,
      (x, y) => null
  ...
  it('should join the two sequences on the selected key', () => {
    ...
    const people1 = enumerable(src([magnus, terry1, adam, john1, john4]));
